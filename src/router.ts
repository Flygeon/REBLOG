...
// P1 阶段：未实现的页面统一指向 _Placeholder.vue（按 route.name 显示标题），
// 后续 Phase 实现真实页面时，把对应路由的 component 替换为真实页面组件即可。
// 注意：本文件只导出 routes 数组，路由实例由 app.ts 按运行环境（web/memory）创建，
// 避免在 Node 端 import 时触发 createWebHistory 访问 window 而报错。
const Placeholder = () => import("@/pages/_Placeholder.vue");
...
const routes: RouteRecordRaw[] = [
  ...
  // 文章详情（slug 可能含 /，用 (.*) 全捕获）
  { path: "/posts/:slug(.*)", name: "post", component: () => import("@/pages/Post.vue") },
  ...
  { path: "/bangumi", name: "bangumi", component: () => import("@/pages/Bangumi.vue") },
  { path: "/memos", name: "memos", component: () => import("@/pages/Memos.vue") },
  { path: "/tags/:tag", name: "tag", component: () => import("@/pages/Tag.vue") },
  { path: "/categories/:category", name: "category", component: () => import("@/pages/Category.vue") },
